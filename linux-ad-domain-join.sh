...
if command -v tee >/dev/null 2>&1; then
    ...
    # Preflight: process substitution can fail in restricted environments (e.g., no /proc/self/fd).
    # Use a subshell so failures do not affect the current shell descriptors.
    if ( : > >(tee -a "$LOG_FILE" >/dev/null) ) 2>/dev/null; then
        # Close lock FD (9) in tee processes to prevent lock inheritance beyond script lifetime.
        # NOTE: FD 9 is the fixed lock FD in this script.
        if command -v stdbuf >/dev/null 2>&1; then
            exec > >(stdbuf -oL tee -a "$LOG_FILE" 9>&-) 2> >(stdbuf -oL tee -a "$LOG_FILE" 9>&- >&2)
        else
            exec > >(tee -a "$LOG_FILE" 9>&-) 2> >(tee -a "$LOG_FILE" 9>&- >&2)
        fi
        ...
        # Fallback: no console mirroring, but keep logging reliable.
        exec >>"$LOG_FILE" 2>&1
    fi
else
    : >"$LOG_FILE" 2>/dev/null || true
...
backup_prune_old_runs() {
    ...
    local tmp_list=""
    # Best-effort: do not abort the entire run if mktemp fails.
    tmp_list="$(mktemp_try "/tmp/linux-ad-domain-join.backup-prune.XXXXXX")" || {
        log_info "ℹ Backup pruning skipped (mktemp unavailable or /tmp not writable)"
        return 0
    }
...
apply_tmout_profile() {
    ...
    # Keep payload deterministic to preserve idempotency (no timestamps).
    payload="$(cat <<EOF
...
if (( ${#missing_pkgs[@]} > 0 )); then
    ...
    #
    # Output policy:
    #   - VERBOSE: print full diagnostic summary (layers + probe details).
    #   - NORMAL : print only the final decision line (no per-layer spam).
    # -------------------------------------------------------------------------
    ...
    # =====================================================================
    # Logging connectivity summary (VERBOSE-only); NORMAL prints only final line
    # =====================================================================
    if $VERBOSE; then
        ...
        if (( ${#CONNECT_DETAILS[@]} > 0 )); then
            _last="${CONNECT_DETAILS[$(( ${#CONNECT_DETAILS[@]} - 1 ))]}"
            _last_s="$(sanitize_log_msg <<< "$_last")"
            _last_c="$(colorize_tag "$_last_s")"
            log_info "${_last_c}"
        fi
    fi
...
for key in passwd shadow group services netgroup; do
    ...
    if grep -Eq "${pattern}" "$NSS_EDIT"; then
        # Preserve ownership/mode when rewriting the file (avoid inode/attr drift).
        _nss_tmp="$(safe_mktemp "${NSS_EDIT}.XXXXXX")"
        awk -v key="${key}" '
            BEGIN {found=0}
            /^[[:space:]]*#/ {print; next}
            tolower($0) ~ "^[[:space:]]*"key":" {
            ...
            {print}
        ' "$NSS_EDIT" >"$_nss_tmp" || { rm -f -- "$_nss_tmp"; log_error "Failed to rewrite NSS work file (dedup) for key=$key" 1; }

        chown --reference="$NSS_EDIT" "$_nss_tmp" 2>/dev/null || true
        chmod --reference="$NSS_EDIT" "$_nss_tmp" 2>/dev/null || true
        mv -f -- "$_nss_tmp" "$NSS_EDIT" || { rm -f -- "$_nss_tmp"; log_error "Failed to install NSS work file (dedup) for key=$key" 1; }
        selinux_restore "$NSS_EDIT"
    fi
...
_nss_tmp="$(safe_mktemp "${NSS_EDIT}.XXXXXX")"
awk '{gsub(/[[:space:]]+/, " "); sub(/^ /, ""); sub(/ $/, "")}1' "$NSS_EDIT" >"$_nss_tmp" \
    || { rm -f -- "$_nss_tmp"; log_error "Failed to normalize whitespace in NSS work file" 1; }
chown --reference="$NSS_EDIT" "$_nss_tmp" 2>/dev/null || true
chmod --reference="$NSS_EDIT" "$_nss_tmp" 2>/dev/null || true
mv -f -- "$_nss_tmp" "$NSS_EDIT" || { rm -f -- "$_nss_tmp"; log_error "Failed to install normalized NSS work file" 1; }
selinux_restore "$NSS_EDIT"
